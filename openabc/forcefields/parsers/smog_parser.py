...
import pandas as pd
import mdtraj
from openabc.utils import get_dataframe_from_pdb, atomistic_pdb_to_ca_pdb, find_cg_pairs_from_atomistic_pdb
...
class SMOGParser(object):
    ...
    def __init__(self, atomistic_pdb, ca_pdb, default_parse=True):
        ...
        self.cg_pdb = ca_pdb
        self.cg_dataframe = get_dataframe_from_pdb(ca_pdb)
        # check if all the atoms are protein CA atoms
        assert ((self.cg_dataframe['resname'].isin(_amino_acids)).all() and self.cg_dataframe['name'].eq('CA').all())
        if default_parse:
    ...
    def parse_mol(self, get_native_pairs=True, frame=0, radius=0.1, bonded_radius=0.05, cutoff=0.6, box=None, 
        ...
        n_atoms = len(self.cg_dataframe.index)
        for atom1 in range(n_atoms):
            chain1 = self.cg_dataframe.loc[atom1, 'chainID']
            if atom1 < n_atoms - 1:
                atom2 = atom1 + 1
                chain2 = self.cg_dataframe.loc[atom2, 'chainID']
                if chain1 == chain2:
                    bonds.append([atom1, atom2])
            if atom1 < n_atoms - 2:
                atom3 = atom1 + 2
                chain3 = self.cg_dataframe.loc[atom3, 'chainID']
                if (chain1 == chain2) and (chain1 == chain3):
                    angles.append([atom1, atom2, atom3])
            if atom1 < n_atoms - 3:
                atom4 = atom1 + 3
                chain4 = self.cg_dataframe.loc[atom4, 'chainID']
        ...
        if get_native_pairs:
            ...
            self.native_pairs = find_cg_pairs_from_atomistic_pdb(
        ...
        # set mass and charge
        for i, row in self.cg_dataframe.iterrows():
            self.cg_dataframe.loc[i, 'mass'] = mass_dict[row['resname']]
            self.cg_dataframe.loc[i, 'charge'] = charge_dict[row['resname']]

...
from openabc.lib import NA, kB, EC, VEP, _amino_acids, _dna_nucleotides, _rna_nucleotides
import math
import sys
import os
...
def all_smog_MJ_3spn2_term(mol, param_PP_MJ, cutoff_PD=1.425*unit.nanometer, force_group=11):
    ...
    """
    # note to self - remove everything related to 6 (the attractive part)
...
# Added by lxc;
def all_smog_NC_term(mol, cutoff=1.6*unit.nanometer, force_group=11):
    """
    SMOG NC potential.
    Many parameters are saved as attributes of mol. 
    ...
    """
    vdwl = mm.CustomNonbondedForce('''energy;
           energy=epsilon_NC*((sigma_NC/r)^12-offset)*step(cutoff-r);
           offset=(sigma_NC/cutoff)^12;         
           epsilon_NC=1.0;
           sigma_NC=0.4;
           cutoff=1.6''')
    ...
    for _, row in mol.atoms.iterrows():
        ...
        if (resname in _amino_acids) and (name == 'CA'):
            ...
            vdwl.addParticle([len(_amino_acids) + _dna_3spn2_atom_names.index(name)])
        elif (resname in _rna_nucleotides) and (name == 'P'):
            vdwl.addParticle([len(_amino_acids) + _rna_atom_names.index(name)])
        else:
            sys.exit(f'Cannot recognize atom with resname {resname} and name {name}.')
    # add exclusions
    for _, row in mol.exclusions.iterrows():

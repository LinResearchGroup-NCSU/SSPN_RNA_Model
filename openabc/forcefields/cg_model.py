...
class CGModel(object):
    ...
    def append_mol(self, new_mol, verbose=False):
        ...
        """
        new_atoms = new_mol.dataframe.copy()
        if hasattr(self, 'dataframe'):
            if self.dataframe is None:
                add_index = 0
                self.dataframe = new_atoms
            else:
                add_index = len(self.dataframe.index)
                self.dataframe = pd.concat([self.dataframe, new_atoms], ignore_index=True)
        else:
            add_index = 0
            self.dataframe = new_atoms
    ...
    def create_system(self, top, use_pbc=True, box_a=500, box_b=500, box_c=500, remove_cmmotion=True):
        ...
        mass = self.dataframe['mass'].tolist()
        for each in mass:

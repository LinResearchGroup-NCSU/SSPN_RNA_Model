...
def find_res_pairs_from_atomistic_pdb(atomistic_pdb, cut_pdb=None, frame=0, radius=0.1, bonded_radius=0.05, cutoff=0.6, box=None, 
    ...
    The algorithm only searches native pairs between residues that do not have 1-2, 1-3, or 1-4 interactions. 
...
def find_cg_pairs_from_atomistic_pdb(atomistic_pdb, cut_pdb=None, frame=0, radius=0.1, bonded_radius=0.05, cutoff=0.6, mol_type='protein', box=None, 
    ...
    mol_type : str
        Accepts 'protein' or 'rna'. Whether to look for ca or phosphorus pairs.
    ...
    if(mol_type == 'protein'):
        for i, row in df_atoms.iterrows():
            if (row['resName'] in _amino_acids) and (row['name'] == 'CA'):
                ...
                dict_res_CG[unique_resSeq] = i
    elif(mol_type == 'rna'):
        for i, row in df_atoms.iterrows():
            if (row['resName'] in _rna_nucleotides_unaltered) and (row['name'] == 'P'):
                unique_resSeq = df_atoms.loc[i, 'unique_resSeq']
                dict_res_CG[unique_resSeq] = i
    df_cg_pairs = pd.DataFrame(columns=['a1', 'a2', 'mu'])
    cg_atom_pairs = []
    for each in res_pairs:
        a1, a2 = int(each[0]), int(each[1])
        if (a1 in dict_res_CG) and (a2 in dict_res_CG):
            df_cg_pairs.loc[len(df_cg_pairs.index)] = [a1, a2, None]
            cg_atom_pairs.append([dict_res_CG[a1], dict_res_CG[a2]])
